...
searchInput.addEventListener('input', function() {
  ...
  if(val===""){
    sectionsAll.forEach((s,i)=>{
      ...
      tabButtonsAll[i].classList.remove("active");
    });
  ...
  if(firstSection){
    ...
    tabButtonsAll[idx].classList.add("active");
  }
});
...
function updateWhatsappLink(){
  ...
  const text = total > 0 ? `Hola! Tengo un pedido de ${formatRD(total)}.` : 'Hola!';
  // using wa.link shortcode: append ?text=
  whatsappBtn.href = `https://wa.link/n7qvjb?text=${encodeURIComponent(text)}`;
}
